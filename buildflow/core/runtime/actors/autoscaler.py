...
class AutoscalerActor:
    ...
    def get_recommended_num_replicas(self, snapshot: ProcessorSnapshot) -> int:
        ...
        if new_num_replicas > self.config.max_replicas:
            if self.config.max_replicas <= max_cluster_replicas:
                logging.warning(
                    "reached the max allowed replicas of %s",
                    self.config.max_replicas,
                )
                new_num_replicas = self.config.max_replicas
        elif new_num_replicas < self.config.min_replicas:
            logging.warning(
                "reached the minimum allowed replicas of %s",
                self.config.min_replicas,
            )
            new_num_replicas = self.config.min_replicas

        if new_num_replicas > max_cluster_replicas:
            if max_cluster_replicas < self.config.max_replicas:
                logging.warning(
        ...
        print('---------------------------------------------------------\n'
              ...
              f'Config Max Replicas: {self.config.max_replicas}\n'
              f'Config Min Replicas: {self.config.min_replicas}\n'
              '---------------------------------------------------------\n')
