...
class PubSubSource(io.StreamingSource):
    ...
    def __init__(self, cloud: Union[str, io.Cloud], name: str,
        ...
        if self.cloud == io.Cloud.GCP:
            ...
            self.cloud_args[
                "subscription"] = f"projects/{project}/subscriptions/{self.name}"
            self.cloud_args["topic"] = f"projects/{project}/topics/{self.name}"
            self._cloud_source = gcp_pubsub_io.GCPPubSubSource(
                **self.cloud_args)
        else:
...
class PubSubSink(io.Sink):
    ...
    def actor(self, remote_fn: Callable, is_streaming: bool):
        return gcp_pubsub_io.PubSubSinkActor.remote(remote_fn,
                                                    self._cloud_sink)
