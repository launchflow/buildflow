...
from buildflow.core.workspace.pulumi.config import PulumiConfig
from buildflow.core.workspace.pulumi.state import PulumiState


@dataclasses.dataclass
class PulumiWorkspace(WorkspaceAPI):
    # PulumiWorkspace implementation fields
    pulumi_workspace_dir: str
    # WorkspaceAPI required fields
    pulumi_state: PulumiState
    pulumi_config: PulumiConfig

    def get_state(self) -> PulumiState:
        return self.pulumi_state
    ...
    def create(cls, pulumi_workspace_dir: str) -> "PulumiWorkspace":
        # create the initial pulumi state
        pulumi_state_path = os.path.join(pulumi_workspace_dir, "state.json")
        pulumi_state = PulumiState.initial()
        pulumi_state.dump(pulumi_state_path)
        ...
        return cls(pulumi_workspace_dir, pulumi_state, pulumi_config)

    @classmethod
    def load(cls, pulumi_workspace_dir: str) -> "PulumiWorkspace":
        utils.assert_path_exists(pulumi_workspace_dir)
        # load the pulumi state
        pulumi_state_path = os.path.join(pulumi_workspace_dir, "state.json")
        pulumi_state = PulumiState.load(pulumi_state_path)
    ...
    def save(self):
        # save the pulumi state
        pulumi_state_path = os.path.join(self.pulumi_workspace_dir, "state.json")
        self.pulumi_state.dump(pulumi_state_path)
