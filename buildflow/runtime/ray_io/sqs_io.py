...
class SQSSource(io.StreamingSource):
    ...
    def get_queue_url(self):
        if not self._queue_url:
            self._queue_url = _get_queue_url(
                self.get_boto_client(), self.queue_name, self.queue_owner_aws_account_id
            )
        ...
        if source._test_sqs_client is not None:
            ...
            if source.region:
                self.sqs_client = boto3.client(
                    service_name="sqs", region_name=source.region
                )
            else:
                self.sqs_client = boto3.client(service_name="sqs")
        self.queue_url = _get_queue_url(
            self.sqs_client, source.queue_name, source.queue_owner_aws_account_id
        )
        ...
        while self.running:
            try:
                ...
                if "Messages" in response:
                    ...
                    await self._send_batch_to_sinks_and_await(response["Messages"])
                    to_delete = []
